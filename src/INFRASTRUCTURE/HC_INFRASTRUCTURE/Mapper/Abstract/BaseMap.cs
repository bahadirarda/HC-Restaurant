...
namespace HC_INFRASTRUCTURE.Mapper.Abstract
{
        public abstract class BaseMap<TEntity> : IEntityTypeConfiguration<TEntity> where TEntity : class, IBaseEntity
        {
            public virtual void Configure(EntityTypeBuilder<TEntity> builder)
            {
                ...
                builder.Property(x => x.CreatorUserName).IsRequired(false).HasMaxLength(96);

                builder.Property(x => x.UpdatedDate).IsRequired(false);
                builder.Property(x => x.UpdaterUserName).IsRequired(false).HasMaxLength(96);

                builder.Property(x => x.DeletedDate).IsRequired(false);
                builder.Property(x => x.DeleterUserName).IsRequired(false).HasMaxLength(96);
            }
        }
}
