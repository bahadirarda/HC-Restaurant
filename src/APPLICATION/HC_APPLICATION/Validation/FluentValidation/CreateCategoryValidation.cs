...
namespace HC_APPLICATION.Validation.FluentValidation
{
    public class CreateCategoryValidation:AbstractValidator<CreateCategoryDTO>
    {
        ...
        {
            RuleFor(x => x.CategoryName).NotEmpty().WithMessage("Category name cannot be left blank.").MinimumLength(2).MaximumLength(96).WithMessage("Minimum character: 2, Maximum character: 96");
